...
},{"./app.js":2,"./jquery.ydisk":3,"./lightbox.js":4,"jquery":5}],2:[function(require,module,exports){
...
;(function (window, document, $, undefined) {
    ...
    $.fn.ydisk = function(options) {
    	...
    	options = $.extend( true, {
                ...
                urlGetList    : 'https://cloud-api.yandex.net/v1/disk/public/resources?public_key=',
                ...
                replacer      : {
                    ...
                    video : '<video id="{id}" class="ydisk-video video-js" data-setup=\'{}\' controls src="{href}"{size}></video>',
                    image : '<a href="{href}"  controls  data-lightbox="photo-1"> <img id="{id}" class="ydisk-image" controls src="{href}"{size}></a>',
					dload : '<a id="{id}" class="ydisk-dload fa fa-cloud-download btn-slide" download href="{href}">{html}</a>',
                    itemLs: '<a id="{id}" class="ydisk-item" href="{href}" data-lightbox="photo-1">{html}</a>',
                    }
                },
                options );
        ...
        $(this).on('onYdiskStart', function(e) {
            ...
            if (url) {
                ...
                if($this.hasClass('get_item_list')){
                    var previewSize = '&preview_size=XL';
                    var limit = '&limit=100';
                    url = options.urlGetList + encodeURIComponent(url) + previewSize + limit;
                }
                else{
                    ...
                    success: function(data) {
                        ...
                        if($this.hasClass('get_item_list')){
                            for(var i=0; i < data._embedded.items.length; i++){
                                replacerTmp = options.replacer.itemLs;
                                if (replacerTmp) {
                                    var id = $this.attr('id') || 'ydisk-replaced-' + new Date().getTime();
                                    replacerTmp = replacerTmp.replace(/\{id\}/g, id);
                                    replacerTmp = replacerTmp.replace(/\{href\}/g, data._embedded.items[i].preview);
                                }
                                replacer += replacerTmp;
                            }
                        }
                        else{
